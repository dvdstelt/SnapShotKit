...
The scale is frozen for the length of a drag and the surface may only grow during one, so the picture never moves while an edge is being dragged. A surface that refits as the canvas grows would take the picture out from under the pointer that is sizing it, and the drag would chase its own tail. When it does have to grow, because the canvas was dragged clean out of it, the window places it by hand so that the capture stays exactly where it is on screen. Letting go settles both: the surface comes back to whatever the canvas and the capture now cover, and the scale to whatever shows all of it, which is the one moment where moving the picture costs nothing.
