...
The window is a stack of full-width bands separated by hairlines: a menu bar, one band carrying both the drawing tools and their settings, the capture on a mat, the recent captures, and a status line with the canvas size and the zoom along the bottom. Each tool is named under its icon, because a row of icons is quick once learned and slow to learn. Nothing floats over the picture. Commands live in the menus and settings live in the band, so saving and exporting do not spend permanent screen space on things pressed once at the end of a session.
...
Every setting is remembered between sessions, in the same file the band's styles live in. Exporting is rarely done once, and a dialog that opens on the defaults every time is one that has to be corrected every time. They are remembered as soon as they are chosen rather than once the file is written, so a picker closed by mistake does not also lose the format and quality just set.
