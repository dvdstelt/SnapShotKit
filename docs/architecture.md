...
**The recent strip hides below the edge unless it is pinned.** It slides up over the picture when the pointer reaches a grip along the picture's lower edge, and away again when the pointer leaves it; pinned, it docks below the picture and the sidebar as it always did. It is moved with a render transform rather than laid out again, so coming and going never resizes anything. Its tiles carry copy location, pin, take off the strip and delete in their corners, and copy in their context menu. An editor opened with nothing on the canvas brings the strip up on its own, since it is then the thing to choose from, and it goes once the pointer has been over it and left. A capture pinned to it stays at the front however many newer ones arrive, and one taken off it stays in the library and comes back when opened from there. Both lists, and whether the strip is pinned, are kept in `editor.json`, applied to what is on disk rather than to the window's own copy so two editor windows do not undo each other.
...
It is geometry, not a pixel edit, for the same reason a crop is. No picture's pixels are touched; the band goes onto the picture, and from then on that picture is drawn in pieces with the band skipped and everything after it shifted up or left by what the band took. Putting the cuts back puts the picture back exactly as it was.

**A cut belongs to the picture it was made in.** The band goes to the topmost picture under the press, is held inside it, and is kept in that picture's own pixels, before any flip, as `ImageAnnotation.Cuts`. Cuts used to run across the whole canvas and take their band out of everything they crossed, which went wrong as soon as there was more than one picture. A cut made in the capture took the same rows out of a picture pasted beside it, and since a band had to follow a picture when the picture moved, or moving the capture would uncover what had been cut, a band across two pictures could only guess which one to follow; moving the pasted one dragged the band through the capture. On the picture, neither can happen. A cut goes wherever its picture goes, is stretched and flipped with it, and nothing else knows it is there.

The picture gets shorter or narrower from its top-left corner and nothing else moves, apart from what stands on it. Whatever is on the part past the band comes up or across with it, since an arrow pointing at something has to go on pointing at it; something whose middle is in the band ends up on the join, and a box across the band keeps both ends and loses what was in between. Anything off the picture stays where it is, and so do the other pictures: they were not cut, and a picture below the one cut does not close up the gap. Moving a picture is different and takes nothing along, because that is a change of mind about where it goes rather than a change to what is in it. Putting the cuts back moves the same things back down.

With the cuts inside the pictures the document has one coordinate system. Everything is measured on the canvas, and `PictureLayout` is the only place that crosses from the canvas into a picture's own pixels, through its crop, its cuts and its flips. `CutLayout` does the closing up for one picture at a time.

A document written while cuts ran across the canvas is moved over as it is opened, and comes out drawn exactly as it was: each band goes to every picture it crossed, and everything past it, pictures, annotations and the canvas, is moved up or across by what it took. The one thing that cannot be reproduced is an annotation that straddled a band, which was drawn in two halves with the middle missing and now keeps its two ends with the band closed between them.
